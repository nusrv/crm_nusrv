...
- Staging configuration validates `APP_URL`, web/API URLs, MariaDB, business timezone, secrets,
  CAPTCHA, and either Redis URL or granular host/port/username/password/database/TLS settings.
...
The staging target is crm.nusrv.com on Plesk 18.0.80 with Node.js 22.23.2 and MariaDB 11.4.7. No
SSH/Plesk deployment access, staging database credentials, or Redis runtime access is available in
the current environment. Therefore the following cannot truthfully be verified yet:
